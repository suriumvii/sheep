...
function mapfunc(data) {
	...
	function mapFormatting(feature,layer) {
		...
		switch(feature.geometry.type.toLowerCase()) {
			case 'point':
				layer.bindPopup('<table class="table table-sm"><thead><tr><th colspan="2" class="bg-success" style="text-align:center;">'+ feature.properties.areainfo +'</th></tr></thead><tbody><tr><th scope="row">Total Lot Area</th><td>'+ feature.properties.totallotarea +'</td></tr></tbody></table>')
				.on('click', itemClick)
				.on('mouseclick', itemMouseClick)
				.on('mouseout', itemMouseOut);	
				break;
			case 'polygon':
				layer.bindPopup('<table class="table table-sm"><thead><tr><th colspan="2" class="bg-success" style="text-align:center;">'+ <?PHP echo $name; ?> +'</th></tr></thead><tbody><tr><th scope="row">Owner</th><td>'+ feature.properties.owner +'</td></tr><tr><th scope="row">Lot Area</th><td>'+ feature.properties.lotarea +'</td></tr><th scope="row">Crop</th><td>' + feature.properties.crop + '</td></tr><th scope="row">Date Planted</th><td>' + feature.properties.dateplanted + '</td></tr><th scope="row">Estimated Harvest Date</th><td>' + feature.properties.dateharvest + '</td></tr><tr><td colspan="2" style="text-align:center"><div id="graph-container"><canvas id="crops-graph" width="100%" height="100"></canvas></div></td></tr><tr><td colspan="2" style="text-align:center"><button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModalNorm"> Edit Information </button><button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#myModalNorm"> Field Chart </button></td></tr></tbody></table>')
				.on('mouseclick', itemMouseClickWithChart)
				//.on('mouseout', itemMouseOut);
				break;
		}	
	...
	function itemMouseClickWithChart(e){
		e.target.openPopup();
		loadChart();
	}
...
$(document).ready( function(){ 
	...
	$.getJSON("http://localhost/redrootcms/crops.geojson", function(data) {
